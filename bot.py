...
بودجه سالانه) را سریع‌تر از سایت‌های دولتی منتشر و تحلیل می‌کنند و به‌عنوان
منبع ثانویه معتبر قابل استفاده‌اند، اما همیشه منابع رسمی دولتی در اولویت اول
باقی می‌مانند. وقتی رقمی را فقط از یک منبع تخصصی (نه دولتی) پیدا کردی:
• اگر چند منبع معتبر مستقل با هم روی همان رقم توافق داشتند، رقم را با ذکر
  منبع (از طریق آیکن، طبق قوانین فرمت‌دهی) ارائه بده.
...
async def _request_ai(input_parts, question_for_history, context, image_mode=False):
    ...
    def build_request(max_tokens, with_tools, force_tool=False):
        ...
        if with_tools and legal:
            # نکته مهم: قبلاً اینجا یک allowed_domains سخت‌گیرانه فقط روی
            # چند دامنه دولتی وجود داشت. چون بسیاری از صفحات .gov.ir به‌خوبی
            # توسط بک‌اند جست‌وجو ایندکس/قابل‌دسترس نیستند، نتیجه اغلب خالی
            # برمی‌گشت و مدل مجبور می‌شد بگوید «پیدا نشد» حتی برای ارقامی که
            # در منابع تخصصی حسابداری/مالیاتی به‌وضوح منتشر شده بودند. به
            # همین دلیل این محدودیت برداشته شده و جست‌وجو در کل وب باز است؛
            # اولویت‌بندی منابع (رسمی دولتی در درجه اول، سپس سایت‌های تخصصی
            # حسابداری/مالیاتی به‌عنوان منبع ثانویه) به‌طور کامل از طریق
            # دستورالعمل‌های AI_SYSTEM_PROMPT کنترل می‌شود، نه یک فیلتر سخت.
            args["tools"] = [{
                "type": "web_search",
