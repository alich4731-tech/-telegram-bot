...
def _remove_source_links(text: str) -> str:
    """Only sanitizes the final text shown to the user.
    Web-search/source data remains untouched during answer generation.
    """
    if not text:
        return ""
    text = re.sub(r"\[[^\]]*\]\(\s*https?://[^)]*\)", "", text, flags=re.I)
    text = re.sub(r"<a\b[^>]*href\s*=\s*[\"\']?https?://[^\"\'>\s]+[^>]*>.*?</a>", "", text, flags=re.I|re.S)
    text = re.sub(r"https?://[^\s<>\]\)]+", "", text, flags=re.I)
    text = re.sub(r"\bwww\.[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}(?:/[^\s<>\]\)]*)?", "", text, flags=re.I)
    text = re.sub(r"(?<![\w@])(?:[a-zA-Z0-9-]+\.)+(?:ir|com|net|org|gov|ac|edu|co|info)(?:/[^\s<>\]\)]*)?", "", text, flags=re.I)
